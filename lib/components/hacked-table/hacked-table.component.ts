...
import { HackedTableColumnComponent } from "../hacked-table-column/hacked-table-column.component";

@Component({
    selector: 'hacked-table',
    templateUrl: 'hacked-table.component.html',
    styleUrls: ['./hacked-table.component.scss']

})

export class HackedTableComponent implements AfterViewInit, OnChanges {
    @ContentChildren(HackedTableColumnComponent) columnComponents: QueryList<HackedTableColumnComponent>;
    @Input() data: any[];
    ...
    ngAfterViewInit(): void {
        setTimeout(() => { this.columns = [].slice.call(this.elementRef.nativeElement.children).filter(e => e.classList.contains('hacked-table-column')) }, 0);
    }
